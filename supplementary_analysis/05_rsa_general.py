...
LLM_SIM_DIR = BIDS_DIR / "derivatives" / "annotations" / "embeddings" / "contextual word embeddings" / f"{LLM_NAME}_similarity_matrix_adj.npz"  # LLM similarity matrix
PLOTS_DIR = PROJECT_ROOT / "reports" / "plots" / "rsa_results" / LLM_NAME  # plots of the RSA results
os.makedirs(PLOTS_DIR, exist_ok=True)
RSA_DATA_DIR = DATA_DIR / "rsa_results" / LLM_NAME  # RSA results DataFrame, bootstrap and permutation test results
os.makedirs(RSA_DATA_DIR, exist_ok=True)

np.random.seed(42)  # for reproducibility of the bootstrapping and permutation tests
#######################################################################
# Data loading
#######################################################################
...
# Get list of neural similarity matrices for all subjects (one file per subject, one matrix per ROI inside)
neural_sim_files = []
...
for file in tqdm(neural_sim_files, desc="Computing RSA correlations"):
    ...
    for roi, neural_matrix in data.items():
        # Compute Spearman correlation between the neural similarity matrix and the LLM similarity matrix
