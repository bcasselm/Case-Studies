...
CLUSTER_FORMING_THRESHOLD = 0.001                               # Voxel-wise threshold for cluster formation in FWE correction; 0.001 is standard; 0.05 is too liberal and creates massive null clusters
P_VALUE_THRESHOLD = 0.01                                        # Threshold for corrected p-values to identify significant clusters (after FWE correction)
FWE_N_ITERS = 5000                                              # Number of iterations for Monte Carlo FWE correction (at least ~1000 for stable p < 0.05)
MIN_PARCEL_SIZE = 0                                             # Minimum size of parcels in voxels (set to 0 to keep all parcels that survive FWE correction)
MIN_UNION_SIZE = 100                                            # Minimum size of parcels in the union parcellation (set to 100 to avoid very small fragmented parcels in the union, since FWE already controls for cluster significance)
...
def load_dataset():
    """
    Load the Neurosynth dataset.
    """
    return fetch_neurosynth(data_dir=str(DOWNLOAD_DIR),
                            version="7",
                            source="abstract",
                            vocab="LDA400",
                            target="mni152_2mm",
...
def create_topic_specific_parcels(ns_data, top_topics):
    """
    ...
    for topic in top_topics.head(TOP_N_TOPICS).index:
        topic_weights = ns_data.annotations[topic]

        threshold = max(topic_weights.quantile(TOPIC_WEIGHT_QUANTILE), MIN_TOPIC_WEIGHT)
        print(f"Topic: {topic}, Threshold: {threshold:.4f}")
        study_ids = ns_data.annotations[ns_data.annotations[topic] > threshold]["id"].tolist()
        print(f"Number of studies: {len(study_ids)}")

        # Run MKDADensity on topic studies
        target_dset = ns_data.slice(study_ids)

        mkda_estimator = mkda.MKDADensity()
        results = mkda_estimator.fit(target_dset)

        # Perform FWE Monte Carlo correction (requires at least ~1000 iterations for stable p < 0.05)
        fwe_corrector = FWECorrector(method="montecarlo", n_iters=FWE_N_ITERS, voxel_thresh=CLUSTER_FORMING_THRESHOLD)
        results_fwe = fwe_corrector.transform(results)
        ...
        if z_map_name is None:
            for key in results_fwe.maps.keys():
                if key == "z":
                    ...
                    break
        if z_map_name is None:
            raise ValueError(f"No association z-map found. Available maps: {list(results_fwe.maps.keys())}")
        ...
        if p_corr_map_name is None:
            raise ValueError(f"No corrected p-map found. Available maps: {list(results_fwe.maps.keys())}")
        z_map = results_fwe.get_map(z_map_name)
        p_corr_map = results_fwe.get_map(p_corr_map_name)

        z_data = np.nan_to_num(z_map.get_fdata(), nan=0.0, posinf=0.0, neginf=0.0)

        # NiMARE outputs corrected p-values as -log10(p) (maps named generally start with "logp")
        is_logp = "logp" in p_corr_map_name.lower()

        if is_logp:
            p_corr_data = np.nan_to_num(p_corr_map.get_fdata(), nan=0.0)
            # If it's a -log10(p) map, p < 0.01 is equivalent to -logp > -log10(0.01) (≈ 2.0)
            significant_positive = (p_corr_data > -np.log10(P_VALUE_THRESHOLD)) & (z_data > 0)
        else:
            p_corr_data = np.nan_to_num(p_corr_map.get_fdata(), nan=1.0)
            significant_positive = (p_corr_data < P_VALUE_THRESHOLD) & (z_data > 0)
        ...
        # export parcel image to disk for RSA
        output_path = os.path.join(OUT_DIR, f"emotion_parcellation_rsa_{topic}.nii.gz")
...
def merge_topic_parcels_to_union(top_topics):
    """
    ...
    mask_paths = []
    for topic in top_topics.head(TOP_N_TOPICS).index:
        p = os.path.join(OUT_DIR, f"emotion_parcellation_rsa_{topic}.nii.gz")
        if os.path.exists(p):
            mask_paths.append(p)
    ...
    final_path = os.path.join(OUT_DIR, "emotion_parcellation_rsa_union.nii.gz")
...
if __name__ == "__main__":
    ...
    # 1. Load Neurosynth dataset
    ns_data = load_dataset()

    # 2. Get top topics related to emotional concept/word representation
    top_topics = get_top_topics(ns_data)

    # 3. Create topic-specific parcels via meta-analysis
    create_topic_specific_parcels(ns_data, top_topics)

    # 4. Merge topic-specific parcels into a union parcellation for RSA
    final_parcellation = merge_topic_parcels_to_union(top_topics)

    # 5. Sanity checks and visualization of the final union parcellation
    run_sanity_checks()
    save_union_visualization(final_parcellation)
