...
Additionally, the `reports` folder contains all visualizations (examples and results) generated from the above analyses, and the `supplementary_analysis` folder contains any additional analyses or figures that bring nuances to the main findings. Also, the requirements for running the code are listed in `requirements.txt`.